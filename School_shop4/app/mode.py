...
class shop(db.Model):
    ...
    leibie_id = db.Column(db.Integer)
    # leibie = db.relationship('leibie', backref='shops')

    shop_name = db.Column(db.String(128), unique=True)
...
class Usershop(db.Model):
    ...
    def check_name(self, shop_name):
        print(self.shop_name, shop_name)
        if self.shop_name == shop_name:
            return 1;
        return 0
